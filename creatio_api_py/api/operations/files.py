...
def download_file(
    ...
    """
    response: Response = api_instance.make_request(  # type: ignore[no-untyped-call]
...
def upload_file(
    ...
    response: Response = api_instance.add_collection_data(collection, data=payload)
    ...
    try:
        response = api_instance.make_request(  # type: ignore[no-untyped-call]
            "POST",
        ...
        if e.response is not None:
            ...
            log_and_print(message, e, api_instance.debug)
        # Delete the file record if the upload fails
        api_instance.delete_collection_data(collection, file_id)
        raise
